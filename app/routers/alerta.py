...
from app.crud.alerta import get_alerta, get_alertas_by_usuario, get_alertas, create_alerta, update_alerta, delete_alerta
...
def read_alertas_by_usuario(usuario_id: int, skip: int = 0, limit: int = 100, db: Session = Depends(get_db)):
...
def create_new_alerta(alerta: AlertaCreate, db: Session = Depends(get_db)):
    return create_alerta(db, alerta)


@router.put("/{alerta_id}", response_model=AlertaRead)
def update_existing_alerta(alerta_id: int, alerta_update: AlertaUpdate, db: Session = Depends(get_db)):
...
def delete_existing_alerta(alerta_id: int, db: Session = Depends(get_db)):
