...
def read_medicamentos_by_usuario(
    usuario_id: int,
    skip: int = 0,
    limit: int = 100,
    db: Session = Depends(get_db),
...
def update_existing_medicamento(
    medicamento_id: int,
...
def delete_existing_medicamento(
    medicamento_id: int,
    db: Session = Depends(get_db),
    user: str = Depends(get_current_user)
):
